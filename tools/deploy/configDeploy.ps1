...
$APPLICATION_LIST = @("Admin", "CDRPreviewWS", "WebSvc", "ProcMgr")
$DEPLOY_BASE = "E:\Content\GateKeeper"

function Main ($sourceLocation) {
    if( -not $sourceLocation ) {
        Write-Host ""
        Write-Host -foregroundcolor "green" "You must specify the location of the files to deploy."
        Write-Host ""
        exit
    }
...
function Deploy ($sourceLocation) {
    ...
    foreach( $app in $APPLICATION_LIST ) {
        ...
        Write-Host "Deploy configuration to: '$destination'"

		Copy-Item $source\*.config $destination
    }
